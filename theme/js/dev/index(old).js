...
var MainConfig = (function () {
  ...
  MainConfig.AJAX_ADMIN_NOTIFICATIONS = "/ajax/AdminMessages";
...
var CommFuncs = (function () {
    ...
    CommFuncs.scrollTo = function () {
        var topVal = 1;
        $('*').is("a[name=smth]") ? topVal=$("a[name=smth]").offset().top : topVal=1;
        $('body').stop().animate({ scrollTop: topVal/*$("a[name=smth]").offset().top*/ - 20 + 'px' }, 500);
    };
...
var ModalWindow = (function () {
    ...
    ModalWindow.prototype.onOpenFn = function (props) {
        ...
        // for different style forms
        props.additionalStyle!=undefined
        ? winContWrapp.removeClass().addClass('mw-win '+props.additionalStyle)
        : winContWrapp.removeClass().addClass('mw-win');
        //
...
var DDMultiAjax = (function () {
    ...
    DDMultiAjax.prototype.showLoadingFn = function (inElm, isRight) {
        ...
        if(isRight){
            var topVal = 1;
            $('*').is(inElm) ? topVal=inElm.offset().top : topVal=1;
            DiLoading.css({ left: inElm.offset().left + inElm.width() - DiLoading.width() - 20, top: topVal/*inElm.offset().top*/ + parseInt((inElm.outerHeight() - DiLoading.outerHeight()) / 2) - 1 });
        }
...
var IndServ = (function () {
    ...
    IndServ.prototype.init = function () {
        var self = this;
        var emptyRes = '.empty-search-result';
        sbjs.init({
        ...
        if (G_PAGE == 'index') {
            var options = {
                url: function (phrase) {
                    ...
                    if($("#CBvacancy").val() == 1)
                    {
                           rusk = MainConfig.AJAX_POST_GETSEARCHVACS +  '?search=' + phrase;
                    } else if($("#CBvacancy").val() == 2) {
                           rusk =  MainConfig.AJAX_POST_GETAPPLIC + '?search=' + phrase;
                    }
                    else  rusk =  MainConfig.AJAX_POST_GETEMPLS + '?search=' + phrase;
                ...
                requestDelay: 1000,
                list: { 
                    onChooseEvent: function () { self.onAutocompleteFn(); },
                    //
                    // empty result event
                    //
                    onShowListEvent: function(){ if($('*').is(emptyRes)) $(emptyRes).remove() },
                    onLoadEvent: function(){
                        var $findListBlock = $('.easy-autocomplete-container'),
                            findItems = $findListBlock.find('.eac-item');                                      
                        if(!findItems.length){
                            !$('*').is(emptyRes) 
                            ? $findListBlock.append("<div class='empty-search-result active'>Ничего не найдено</div>")
                            : $(emptyRes).addClass('active');
                        }
                    },
                }
            };
        ...
        var objSliderApp = null;
        var objSliderEmp = null;
        $(window).on('load resize',function(e){
            var w = $(window).width(),
                $sliderApp = $("#DiEmplSlider"),
                $sliderEmp = $("#DiEmpl1Slider");
                props = {
                    navigation: true,
                    slideSpeed: 300,
                    pagination: true,
            ...
            if(w >= (768 - 0)){
                if($sliderApp.hasClass('off') || e.type=='load'){
                    ...
                    $sliderApp.owlCarousel(props);
                    $sliderApp.removeClass('off');
                    objSliderApp = $sliderApp.data('owlCarousel');
                }
                if($sliderEmp.hasClass('off') || e.type=='load'){
                    props.singleItem = false;
                    props.items = 6;
                    props.itemsDesktop = [1182, 4];
                    props.itemsDesktopSmall = false;
                    props.itemsTablet = [974, 3];
                    props.itemsTabletSmall = false;
                    $sliderEmp.owlCarousel(props);
                    $sliderEmp.removeClass('off');
                    objSliderEmp = $sliderEmp.data('owlCarousel');
                }
            }
            else{

                if(!$sliderApp.hasClass('off')){
                    if(typeof objSliderApp!=='undefined' && objSliderApp!=null)
                        objSliderApp.destroy();
                    $sliderApp.addClass('off');
                }
                if(!$sliderEmp.hasClass('off')){
                    if(typeof objSliderEmp!=='undefined' && objSliderEmp!=null)
                        objSliderEmp.destroy();
                    $sliderEmp.addClass('off');
                }
            }
        });
        /*var props = {
        ...
        $('.search-category').click(function(e){ 
            $.each($('.search-category'), function(e){ $(this).removeClass('active'); });
            $(this).addClass('active');
            $('#CBvacancy').val($(this).data('val'));
            self.onChangeSearchTypeFn(e, $('#CBvacancy'));
            $(this).data('val')==3 ? $('.search__wrd-module').addClass('disable') : $('.search__wrd-module').removeClass('disable');
        });
        //
        //  example search word
        //
        $('.search-word').click(function(e){ 
            ...
            if($('*').is(emptyRes)) $(emptyRes).remove();
        });
        //
        //  check for empty result
        //
         $("#EdSearch").focusin(function(){
            if($('*').is(emptyRes)){ $(this).val()!='' ? $(emptyRes).addClass('active') : $(emptyRes).removeClass('active') }
        });
        $("#EdSearch").focusout(function(){ if($('*').is(emptyRes)) $(emptyRes).removeClass('active') });
    ...
    IndServ.prototype.showLoading = function (inElm, isRight, props) {
        ...
        DiLoading.find('img').attr('src', src).one('load', function () {
            ...
            var topVal = 1;
            $('*').is(inElm) ? topVal=inElm.offset().top : topVal=1;
            DiLoading.css({ top: topVal/*inElm.offset().top*/ + parseInt((inElm.outerHeight() - DiLoading.outerHeight()) / 2) - 1 });
            if (isRight)
                DiLoading.css({ left: inElm.offset().left + inElm.width() - DiLoading.width() - 20, top: topVal/*inElm.offset().top*/ + parseInt((inElm.outerHeight() - DiLoading.outerHeight()) / 2) - 1 });
            else if (align == 'center')
                DiLoading.css({ left: inElm.offset().left + (inElm.outerWidth() - DiLoading.outerWidth()) / 2 });
            else
                DiLoading.css({ left: inElm.offset().left + offsetLeft, top: topVal/*inElm.offset().top*/ + parseInt((inElm.outerHeight() - DiLoading.outerHeight()) / 2) - 1 });
            if (props && props.left)
                DiLoading.css({ left: inElm.offset().left + props.left });
            if (props && props.top)
                DiLoading.css({ top: topVal/*inElm.offset().top*/ });
            DiLoading.fadeIn(400);
        });
    };
    IndServ.prototype.showLoading2 = function (inElm, props) {
        ...
        DiLoading.find('img').attr('src', src).one('load', function () {
            ...
            $('*').is(inElm) ? topVal=inElm.offset().top : topVal=1;
            if (props.valign == 'top')
                DiLoading.css({ top: topVal/*inElm.offset().top*/ + props.offsetY });
            else if (props.valign == 'middle')
                DiLoading.css({ top: topVal/*inElm.offset().top*/ + parseInt((inElm.outerHeight() - DiLoading.height()) / 2) });
            if (props.withBg) {
                if (props.pic == '1')
    ...
    IndServ.prototype.showHint = function (item, inText) {
        ...
        var topVal = 1;
        $('*').is(item) ? topVal=item.offset().top : topVal=1;
        msgBox.text(inText).css({ left: item.offset().left, top: topVal/*item.offset().top*/ + item.outerHeight() + 10 });
    ...
    IndServ.prototype.onAutocompleteFn = function () {
        ...
        if ($("#CBvacancy").val() == 3) {
           ...
           window.location.href = MainConfig.PAGE_SEARCH_PROMO + "/" + value.code; 
    ...
    IndServ.prototype.onChangeSearchTypeFn = function (e, that) {
        ...
        EdSearch.attr('placeholder', EdSearch.data('ph' + val));
    };
    return IndServ;
}());
...
var PageRegister = (function (_super) {
    ...
    PageRegister.prototype.onF1registerSubmitFn = function (e, that) {
        ...
        if (flagError) {
            ...
            var topVal = 1;
            $('*').is(item) ? topVal=item.offset().top : topVal=1;
            $('body').stop().animate({ scrollTop: topVal/*item.offset().top*/ - 30 + 'px' }, 500, function () {
                var msgBox = $('.error-hint-box');
                msgBox.text(mess).css({ left: item.offset().left, top: topVal/*item.offset().top*/ + item.outerHeight() + 10 });
                msgBox.stop().fadeIn(400);
        ...
        $(".affective-perc").hover(function (e) {
            self.showHintBox($(this), { content: "Чтобы повысить эффективность своего профиля: заполните как можно больше полей в своём профиле. Для этого нажмите кнопку редактирования профиля",
                posFunc: function (sourceObj, hintBox) {
                    ...
                    var topVal = 1;
                    $('*').is(sourceObj) ? topVal=sourceObj.offset().top : topVal=1;
                    hintBox.css({ top: topVal/*sourceObj.offset().top*/ + sourceObj.height() + 20 });
                }
            });
        }, function () {
        ...
        if (state) {
            ...
            if (props.posFunc) {
                ...
                if (props.positon == 'bottom') {
                    var topVal = 1;
                    $('*').is(inObj) ? topVal=inObj.offset().top : topVal=1;
                    hintBox.css({
                        left: inObj.offset().left + inObj.outerWidth() - hintBox.outerWidth(),
                        top: topVal/*inObj.offset().top*/ + inObj.height() + 20
                    });
                }
                else {
                }
...
var PageCompanyServices = (function () {
    ...
    PageCompanyServices.prototype.onMenuClickFn = function (e, that) {
        ...
        var topVal = 1;
        $('*').is("a[name=" + hash + "]") ? topVal=$("a[name=" + hash + "]").offset().top : topVal=1;
        $('body').stop().animate({ scrollTop: topVal/*$("a[name=" + hash + "]").offset().top*/ - 20 + 'px' }, 500);
    };
...
var PageServices = (function (_super) {
    ...
    PageServices.prototype.init = function () {
        ...
        // registered users
        $(".page-services .services__item-block").click(function (e) { self.onServiceInDevClickFn(e, this); });
        $(".page-services .services__sub-item").click(function (e) { self.onServiceInDevClickFn(e, this); });
        //
    ...
    PageServices.prototype.onServiceInDevClickFn = function (e, that){
        var $that = $(that);
...
var PageVacancyView = (function () {
    ...
    PageVacancyView.prototype.init = function () {
        ...
        if (QueryString['tab']){
            var topVal = 1;
            $('*').is(".tabs-panel") ? topVal=$(".tabs-panel").offset().top : topVal=1;
            $('body').stop().animate({ scrollTop: topVal/*$(".tabs-panel").offset().top*/ - 20 + 'px' }, 10);
        }
...
var ResponsesApplic = (function () {
    function ResponsesApplic() {
        var self = this;
        self.VacResponses = new VacResponses();
        $(".controls .js-cancel").click(function (e) { self.onChangeStatusCLickFn(e, this, 3); });
        $(".controls .apply").click(function (e) { self.onChangeStatusCLickFn(e, this, 5); });
        self.init();
    }
...
$(document).ready(function () {
    ...
    /*
    *   show message to transition on playmarket
    */
    /*if(typeof $.cookie('show_mess_mob') == 'undefined')
        $.cookie('show_mess_mob', 0);

    if(device.mobile() && $.cookie('show_mess_mob')==0){
        $.ajax({
            url: MainConfig.AJAX_GET_SHOW_MESS,
            success: function(res){ 
                $('body').append(res);

                var apm = '.app-message',
                    $apmBody = $('body').find(apm),
                    $apmVeil = $('body').find(apm+'__veil');

                $apmVeil.fadeIn();
                setTimeout(function(){ $apmBody.fadeIn() },500);

                $(window).on('load scroll resize',function(e){
                    var $mainMenu = $('#DiTopMenuWrapp'),
                        hMessBody = $apmBody.height(),
                        pMessBody = $apmBody.offset().top,
                        pMainMenu = $mainMenu.offset().top + $mainMenu.height(),
                    ...
                    if(scrollPos>0){
                        $apmVeil.css({top:(pMainMenu-scrollPos)});
                        if(scrollPos>pMainMenu){ // крутим, если ниже меню
                        ...
                        else{ $apmBody.css({ top:pMainMenu }) }
                    }
                    else{ 
                        $apmVeil.css({ top:pMainMenu });
                        $apmBody.css({ top:pMainMenu });
                    }
                    if(e.type==='load') 
                        scrollPos>0 ? $apmBody.css({ top:scrollPos }) : $apmBody.css({ top:pMainMenu });
                });

                $apmBody.on('click', apm+'__close', function(){
                    $apmBody.fadeOut();
                    $apmVeil.fadeOut();
                    $.cookie('show_mess_mob', 1);// already show                    
                });
        ...
        $.get(MainConfig.AJAX_GET_GETUSERNEWMESSAGES, {}, function (data) {
            try {
                ...
                if (data['newmessages'].length) {
                    var noticesCnt = 0;
                    $.each(data['newmessages'], function(){ noticesCnt+=Number(this['count']) });
                    $('#sm-notice-cnt b').html(noticesCnt);
...
var InputFields = (function () {
    ...
    InputFields.prototype.checkPhone = function(elem){
        $this = $(elem);
        if($this.val().replace(/\D+/g,'').length==11){//11 символов в номере телефона
    ...
    InputFields.prototype.checkEmail = function(elem){
        $this = $(elem);
        if($this.val() != '') {
    ...
    InputFields.prototype.setPhoneMask = function(elem){
