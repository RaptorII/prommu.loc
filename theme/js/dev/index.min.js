    ...
    return t.prototype.init = function() {
        ...
        if($('.prommu_flash').is('*'))
        {
            $.fancybox.open({
                ...
                type: 'inline',
                touch: false,
                afterClose: function(){ $('body>div.prommu_flash').remove() }
            })            
        }
    }
    ,
...
$(document).ready(function() {
    ...
    //
    // проверяем наличие уведомлений от админа
    //
    ...
    setTimeout(function(){
      $.ajax({
        ...
        success: function (r)
        {
          ...
          {
            arAdminNotifications = r.items;
            $('body').append('<div class="admin_notifications">'
            //  + '<div class="hint">Заказать обратный звонок</div>'
    ...
    // закрываем или открываем список уведомлений
    $('body').on('click','.admin_notifications',function(){
      if(!$(this).hasClass('active')) // добавляем
      {
        var html = '<div class="admin_notifications_veil"></div><div class="admin_notifications_list">';
        $.each(arAdminNotifications, function(){
          html += '<span class="item" data-id="' + this.id_message + '">' + this.title + '</span><br>';
        });
        html += '</div>';
        $('body').append(html);
        $(this).addClass('active');
    ...
    .on('click','.admin_notifications_list .item',function(){
        ...
        $.each(arAdminNotifications,function(){
            if(this.id_message==id)
            {
              var html = '<div class="admin_notifications_mess prmu__popup">'
                    + '<h3>' + this.title + '</h3>'
                    + '<p>' + this.text + '</p>'
                    + '<span data-id="' + this.id_message + '" class="admin_notifications_agree">ХОРОШО</span>'
                    + '</div>';
              $('body').append(html);
              $('.admin_notifications_list').fadeOut();
              ...
              $.fancybox.open({
                ...
                afterClose: function(){
                    ...
                    $('.admin_notifications_list').fadeIn();
                    $('.admin_notifications').fadeIn();
                }
              })
            }
    ...
    .on('click','.admin_notifications_agree',function(){
      ...
      $('.admin_notifications_list').remove();
        $.each(arAdminNotifications,function (i,e) {
          if(this.id_message==id)
          { arAdminNotifications.splice(i, 1); }
        });

        $.ajax({
            url: MainConfig.AJAX_ADMIN_NOTIFICATIONS,
            dataType: 'json',
            data: {data:JSON.stringify({agree:id})}
        });

        if(arAdminNotifications.length)
        {
          var html = '<div class="admin_notifications_list">';
          $.each(arAdminNotifications, function(){
            html += '<span class="item" data-id="' + this.id_message + '">' + this.title + '</span><br>';
          });
          html += '</div>';
          setTimeout(function(){ $('body').append(html); }, 500);
          $('.fancybox-close-small').click();
        }
        else
        {
          $('.admin_notifications').remove();
