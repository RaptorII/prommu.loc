...
var __extends = this && this.__extends || function(t, e) {
  ...
  return t.SITE = "",
    ...
    t.AJAX_ADMIN_NOTIFICATIONS = "/ajax/AdminMessages",
    t
}()
  , CommFuncs = function() {
  function t() {}
...
*/ // ввод нового города
      ...
      var t = {
        url: function(t) {
          var arCities = $('.cities-select [name="cities[]"]'),
              strCities = '';

          $.each(arCities,function(){
            strCities += '&cities[]=' + $(this).val();
          })
          return (1 == $("#CBvacancy").val()
            ? MainConfig.AJAX_POST_GETSEARCHVACS + "?search=" + t
            : 2 == $("#CBvacancy").val() ? MainConfig.AJAX_POST_GETAPPLIC + "?search=" + t : MainConfig.AJAX_POST_GETEMPLS + "?search=" + t)
            + strCities;
        },
        getValue: "name",
        ajaxSettings: {
    ...
    $(window).on("load resize", function(t) {
      ...
      props = {
        ...
        751 <= e ? (!n.hasClass("off") && "load" != t.type || (props.singleItem = !1,
          props.items = 6,
  ...
  return t.prototype.bindFiltersFn = function(t) {
    ...
    ,
    t.prototype.onFormSubmit = function(t, e, n) {
      (n = n || {}).onBeforeSubmit && (n.inJustReturn = 1),
        n = n || {};
...
var PageApplicantList = function() {
  ...
  return t.prototype.init = function() {
    ...
    t.prototype.onChangeStatusCLickFn = function(t, e, n) {
      var i = $(e).closest(".row")[0]
        , o = $(e).closest(".controls")[0];
      if (t.preventDefault(),
        (3 == n || 5 == n) && !confirm("Подтвредите действие"))
        return 0;
      $(".content-block").addClass("load"),
        $.post(MainConfig.AJAX_POST_SETRESPONSESTATUS, {
          ...
          if (t = JSON.parse(t),
              ...
              t.error)
            return 0;
          1 == n ? ($(i).removeClass("-new"),
            $(i).find(".label-new").remove(),
            $(e).fadeOut()) : 3 == n ? $(i).fadeOut() : 5 == n && ($(i).removeClass("-new"),
            $(i).find(".label-new").remove(),
            $(o).html('<div class="status">Заявка на вакансию подтверждена обеими сторонами</div>'))
        })
    }
    ,
    t
      ...
      $(".content-block").addClass("load"),
        $.post(MainConfig.AJAX_POST_SETRESPONSESTATUS, {
          ...
          if (t = JSON.parse(t),
              ...
              t.error)
            return 0;
          3 == n ? $(i).fadeOut() : 5 == n && ($(i).removeClass("-new"),
            $(i).find(".label-new").remove(),
            $(o).html('<span class="status">Подтверждена обеими сторонами</span>'))
        })
    }
    ,
    t
  ...
  return __extends(t, Page),
    t.prototype.onQuestClick = function(t, e) {
      var n = $(e);
...
G_VARS.App = new IndServ,
  ...
  $(document).ready(function() {
    ...
    "page" == G_PAGE && "faq" == G_ACTION_ID && new CommonPages,
      ...
      setTimeout(function() {
        $.ajax({
          ...
          dataType: "json",
          success: function(t) {
            !t.error && t.items.length && (arAdminNotifications = t.items,
              $("body").append('<div class="admin_notifications"><div class="circlephone"></div><div class="circle-fill"></div><div class="img-circle"><div class="img-circleblock"></div></div></div>'),
              $(".admin_notifications").fadeIn())
          }
        })
      }, 1e3),
      $("body").on("click", ".admin_notifications", function() {
        if ($(this).hasClass("active"))
          $(".admin_notifications").removeClass("active"),
            $(".admin_notifications_list").remove(),
            $(".admin_notifications_veil").remove();
        else {
            ...
            if (this.id_message == i) {
              ...
              $("body").append(n),
                ...
                $.fancybox.open({
                  ...
                  afterClose: function() {
                    $(".admin_notifications_mess").remove(),
                      arAdminNotifications.length ? ($(".admin_notifications_list").fadeIn(),
                        $(".admin_notifications").fadeIn()) : ($(".admin_notifications").remove(),
                        $(".admin_notifications_veil").fadeOut())
                  }
                }),
                arAdminNotifications.splice(t, 1),
                $.ajax({
                  url: MainConfig.AJAX_ADMIN_NOTIFICATIONS,
                  dataType: "json",
                  data: {
    ...
    {
      var personalMenu = $('.personal-acc__menu'),
          posAccMenu = $(personalMenu).offset().top - 100;
      ...
      function scrollAccMenu()
      {
        (
          ...
          ? $(personalMenu).addClass('fixed')
          : $(personalMenu).removeClass('fixed');
      }
    }
    ...
    t.prototype.getUserNewMessages = function() {
      $.get(MainConfig.AJAX_GET_GETUSERNEWMESSAGES, {}, function(t) {
        try {
          ...
          0 < (e += Number(t.vacancy_public_mess_cnt)) && ($("#sm-notice-cnt b").html(e),
  ...
  return t.buttonLoading = function(t, e) {
    if (1 == e) {
      ...
      s < 0 && (s = 0),
        ...
        t.dataset.content = n,
        $(t).html('<div class="btn-loading"><div></div><div></div><div></div><div></div></div>'),
        $(t).find(".btn-loading").css({
          height: o,
          left: s
        }),
        $(t).find(".btn-loading>div").css({
          top: a
        })
    }
    0 == e && (n = t.dataset.content,
      $(t).html("").html(n))
  }
    ,
    ...
    t.stateLoading = function(flag) {
      if(flag==true)
      {
        if(!$('*').is('.prmu-load'))
        {
          $('body').append('<div class="prmu-load">');
        }
      }
      else
      {
