...
var PageApplicantList = function() {
  ...
  , ResponsesCompany = function() {
    ...
    t.prototype.onChangeStatusCLickFn = function(t, e, n) {
        var i = $(e);
        if (t.preventDefault(),
        (3 == n || 4 == n) && !confirm("Подтвредите действие"))
            return 0;
        $.post(MainConfig.AJAX_POST_SETRESPONSESTATUS, {
            idres: i.closest(".controls").data("sid"),
            s: n
        }, function(t) {
            if (!(t = JSON.parse(t)).error)
                if (1 == n)
                    i.closest(".row").removeClass("-new"),
                    i.fadeOut(300, function() {
                        $(this).parent().remove()
                    });
                else if (3 == n)
                    i.closest(".row").fadeOut(300, function() {
                        $(this).remove()
                    });
                else if (4 == n) {
                    var e = i.closest(".controls");
                    i.closest(".row").removeClass("-new"),
                    e.find(".view").parent().fadeOut(300, function() {
                        $(this).remove()
                    }),
  ...
  , ResponsesApplic = function() {
    ...
    ,
    t.prototype.onChangeStatusCLickFn = function(t, e, i) {
        var o = $(e);
        if (t.preventDefault(),
        -1 < $.inArray(i, [3, 5]) && !confirm("Подтвредите действие"))
            return 0;
        $.post(MainConfig.AJAX_POST_SETRESPONSESTATUS, {
            idres: o.closest(".controls").data("sid"),
            s: i
        }, function(t) {
            if (!(t = JSON.parse(t)).error)
                if (5 == i) {
                    var e = o.closest(".controls");
                    o.closest(".row").removeClass("-new"),
                    o.parent().fadeOut(300, function() {
                        e.find(".status").removeClass("hint").attr("title", "").text(o.data("status")),
                        $(this).remove()
                    }),
                    ModalWindow.open({
                        content: t.message
                    }),
                    e.find(".js-cancel").fadeOut(200, function() {
                        $(this).remove()
                    }),
                    e.find(".js-applied").fadeIn(400)
                } else if (3 == i) {
                    var n = o.closest(".row");
  ...
  , PushChecker = function() {
    ...
    t.prototype.getUserNewMessages = function() {
        $.get(MainConfig.AJAX_GET_GETUSERNEWMESSAGES, {}, function(t) {
            try {
                if ((t = JSON.parse(t)).newmessages.length) {
                    var e = 0;
                    $.each(t.newmessages, function() {
                        e += Number(this.count)
                    }),
