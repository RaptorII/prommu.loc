$(function(){
	...
	if(block==1){
	    var titleLen = 70,//  Заголовок не более 70и символов
	        ...
	        arSelect = ['vacancy','expirience', 'hcolor', 'hlen', 'ycolor', 'chest', 'waist', 'thigh'];
	        arAttr = ['hcolor', 'hlen', 'ycolor', 'chest', 'waist', 'thigh'];
	    ...
	    $(document).on('click', function(e){
	      for(var i=0; i<arSelect.length; i++){
	        var $list = $('#rv-'+arSelect[i]+'-list');
	        if(e.target.id == 'rv-'+arSelect[i]+'-veil'){
	          $list.fadeIn();
	          $list.closest('.erv__label').addClass('focus');
	          $list.closest('.erv__label-half').addClass('focus');
	        }
	        else if($(e.target).is('#rv-'+arSelect[i]+'-list i') || !$(e.target).closest('#rv-'+arSelect[i]+'-list').length){
	          $list.fadeOut();
	          $list.closest('.erv__label').removeClass('focus');
	        }
	      }
	    });
		/*
		...
		*/  // новый ввод вакансий
	    ...
	    $('#rv-expirience-list input').on('change', function(){
	      ...
	      if($this.is(':checked')){
	        ...
	        $this.val()=='2' ? $('#rv-exinfo-label').fadeIn() : $('#rv-exinfo-label').fadeOut();
	      }
	      $('#rv-expirience-list').fadeOut();
	      checkField('#rv-expirience');
	    });
	    // атрибуты
	    $('#rv-hcolor-list input').on('change', function(){ checkSetAttr(this, 'hcolor') });
	    $('#rv-hlen-list input').on('change', function(){ checkSetAttr(this, 'hlen') });
	    $('#rv-ycolor-list input').on('change', function(){ checkSetAttr(this, 'ycolor') });
	    $('#rv-chest-list input').on('change', function(){ checkSetAttr(this, 'chest') }); 
	    $('#rv-waist-list input').on('change', function(){ checkSetAttr(this, 'waist') });
	    $('#rv-thigh-list input').on('change', function(){ checkSetAttr(this, 'thigh') });
	    ...
	    $('#rv-user-height, #rv-user-weight').on('keyup', function(){
	      var val = $(this).val().replace(/\D+/g,'');
	      if(val.length>hwLen) 
	    ...
	    // события подсказок
	    $('.erv__input').focus(function(){ $(this).closest('.erv__label').addClass('focus') });
	    $('.erv__input').blur(function(){ $(this).closest('.erv__label').removeClass('focus') });
	    //    Проверка полей
	...
	if(block==2){
		...
		// события списка сроков оплаты
		$(document).on('click', function(e){
			if(e.target.id == 'rv-paylims-veil'){
				$(payl+'-list').fadeIn();
				$(payl+'-list').closest('.erv__label').addClass('focus');
			}
			else if($(e.target).is(payl+'-list i') || !$(e.target).closest(payl+'-list').length){
				$(payl+'-list').fadeOut();
				$(payl+'-list').closest('.erv__label').removeClass('focus');
			}
		});
		// сроки оплаты
		$(payl+'-list').on('change', 'input', function(){
			$(payl).val($(this).data('name'));
			if($(this).val()=='164')
		...
		//    Проверка полей
		$('.erv__required').bind('keyup', function(){ checkField(this) });
		//
		$('.erv__button').click(function(e){
			e.preventDefault();
			var errors = false;
	...
	//
	function checkSetAttr(e, type){
		if($(e).is(':checked'))
			$('#rv-' + type).val($(e).data('name'));
		$('#rv-' + type + '-list').fadeOut();
		$('#rv-' + type + '-list').closest('.erv__label-half').removeClass('focus');  
	}
	//  
	function addErr(e){ 
		$(e).addClass('error');
