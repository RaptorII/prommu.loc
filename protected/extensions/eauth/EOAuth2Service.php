...
abstract class EOAuth2Service extends EAuthServiceBase implements IAuthService {
	...
	protected function getCodeUrl($redirect_uri) {
		...
		if($this->providerOptions['authorize'] == 'https://oauth.vk.com/authorize&v=5.74' || $this->providerOptions['authorize'] == 'https://www.facebook.com/dialog/oauth'){
			return $this->providerOptions['authorize'] . '?client_id=' . $this->client_id . '&redirect_uri=' . urlencode($redirect_uri) . '&scope=' . $this->scope .',email&response_type=code';
		}else 
	...
	protected function getTokenUrl($code) {
		// $rest = $this->providerOptions['access_token'] . '?client_id=' . $this->client_id . '&client_secret=' . $this->client_secret . '&code=' . $code . '&redirect_uri=' . urlencode($this->getState('redirect_uri')).'&v=V';
		// file_put_contents('/home/prommudev/protected/extensions/telegram.txt', date('d.m.Y H:i')."\t".$rest."\n", FILE_APPEND | LOCK_EX);
