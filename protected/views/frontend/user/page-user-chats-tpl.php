  ...
  <div class='col-xs-12'>
    <?php if(!$viData['count']): ?>
      <?php if(Share::$UserProfile->type==2): // соискатель ?>
        <div class="without-chats">У Вас еще нет диалога ни с одним из Работодателей.<br> Для начала общения попробуйте отозваться на понравившуюся вакансию</div>
      <?php else: ?>
        <div class="without-chats">У Вас еще нет диалога ни с одним из Соискателей.<br> Для начала общения создайте вакансию, найдите в списке анкет соискателей подходящего кандидата и напишите ему</div>
      <?php endif; ?>
    <?php else: ?>
      ...
      <?php foreach ($viData['chats'] as $key => $val): ?>
        <div class='chat-list-block clearfix'>
          ...
          <div class='img-block'>
            <a class='img' href='<?= MainConfig::$PAGE_IM . DS . $val['id'] ?>'>
            ...
            <div class='title'>
              <a href='<?= MainConfig::$PAGE_IM . DS . $val['id'] ?>'><?= $val['title'] ?: $val['etitle'] ?></a>
            </div>
            <div class='fio'>
              <?php if( Share::$UserProfile->type == 2 ): ?>
                <?= $val['name'] ?> (
