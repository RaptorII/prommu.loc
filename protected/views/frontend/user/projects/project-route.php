...
<div class="project__module">
    <?php if (sizeof($viData['items']) > 0): ?>
        ...
        foreach ($viData['filter']['cities'] as $id => $c)
            $arFilterData['FILTER_SETTINGS'][1]['DATA'][$id] = ['title' => $c['city'], 'id' => $id];
        ...
        foreach ($viData['filter']['metros'] as $id => $metro) {
            $arFilterData['FILTER_SETTINGS'][6]['DATA'][$id] = [
                'title' => $metro['metro'],
                'id' => $metro['id'],
        ...
        <div class="project__route-header">
            <div class="project__addr-xls">
                <a  class="add-xls-change" href="javascript:void(0)">Изменить адресную программу</a>
                <a href="/user/uploadprojectxls?id=<?= $project ?>&type=index">Скачать существующую</a>
        ...
        <div class="notification">
            * Добавить новые Города или ТТ для посещения возможно только через раздел
            <a href="/user/projects/<?=$project?>/index">"Адресная программа"</a>
        </div>
