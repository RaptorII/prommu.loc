<?php 
    ...
    switch($info){
        ...
        case 'reject': $title="Отклоненные заявки"; break; // отклоненные
...
<div class="tabs-block">
  <a href="?info=approv" class="tabs-block__link<?=($info=='approv'?' active':'')?>">Утвержденные<b><?=($arResp['counts'][8] ? "({$arResp['counts'][8]})" : "")?></b></a>
  <a href="?info=resp" class="tabs-block__link<?=($info=='resp'?' active':'')?>">Откликнувшиеся<b><?=($arResp['counts'][4] ? "({$arResp['counts'][4]})" : "")?></b></a>
  <a href="?info=aside" class="tabs-block__link<?=($info=='aside'?' active':'')?>">Отложенные<b><?=($arResp['counts'][1] ? "({$arResp['counts'][1]})" : "")?></b></a>
  <a href="?info=reject" class="tabs-block__link<?=($info=='reject'?' active':'')?>">Отклоненные<b><?=($arResp['counts'][3] ? "({$arResp['counts'][3]})" : "")?></b></a>
  <a href="?info=refuse" class="tabs-block__link<?=($info=='refuse'?' active':'')?>">Отказавшиеся<b><?=($arResp['counts'][5] ? "({$arResp['counts'][5]})" : "")?></b></a>
  <a href="?info=dialog" class="tabs-block__link<?=($info=='dialog'?' active':'')?>">Чат вакансии<b><?=($arResp['countsDiscuss'] ? "({$arResp['countsDiscuss']})" : "")?></b></a>
</div>
                ...
                <tbody>
                    <?php foreach ($arResp['responses'][$respInd] as $key => $val): ?>
                        <?php 
                            ...
                            $r = $val['isresponse'];
                        ?>
                        <tr class="table__row" data-sid="<?=$val['sid']?>" data-status="<?=$val['status']?>" data-resp="<?=$val['isresponse']?>">
                            <td class='table__cell'><img src="<?=$val['logo']?>" alt="<?=$name?>" class="responses__logo"></td>
