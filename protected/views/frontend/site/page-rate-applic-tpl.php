...
<?php else: ?>
  <div class='row'>
    <div class='col-xs-12'>
        <?php if( $IS_OWN ): ?>
        <?php else: ?>
            <h2>Рейтинг пользователя <?= $Profile->exInfo->fio ?></h2>
        <?php endif; ?>
      ...
      <br />
      <p>Рейтинг Соискателя показывает качественный уровень выполнения задач проектов, в которых участвовал данный Соискатель. Рейтинг выставляется Работодателем после завершения работы по вакансии, и вычисляется по всем вакансиям, на которых Соискатель завершил работу</p>
      <br />
      <div class='row'>
        <div class='col-xs-12'>
          <table class='rate'>
      ...
      <?php if( $IS_OWN && $viData['rateByUser'] ): ?>
          <div class='header-021 -green'>
            Рейтинг выставленный работодателями
          </div>
          <br />
          <div class='row'>
            <?php foreach ($viData['rateByUser'] as $key => $val): ?>
                    ...
                    <div class="rate-wrapper col-xs-12 col-sm-6 col-lg-4">
                        <div class="rate-block applic clearfix <?= array_values($val)[0]['new'] < 0 ? '-new' : '' ?> <?= in_array($debug, [1,2]) ? '-new':'' ?>">
                            <div class="inner">
                                ...
                                <div class="logo">
                                    <a href="<?= MainConfig::$PAGE_PROFILE_COMMON . DS . array_values($val)[0]['idus'] ?>">
                                        <img src="<?= $this->ViewModel->getHtmlLogo(array_values($val)[0]['logo'], ViewModel::$LOGO_TYPE_EMPL) ?>" alt="">
                                    </a>
                                </div>
                                <div class="company">
                                    <div class="fio">
                                        <a href="<?= MainConfig::$PAGE_PROFILE_COMMON . DS . array_values($val)[0]['idus'] ?>" class="black-green">
                                            <?= array_values($val)[0]['name'] ?>
                                        </a>
                                    </div>
                                    <div class="rates">
