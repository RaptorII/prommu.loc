<img style="padding-top: : -24px; padding-left: 44%;" src="/admin/logo-sm.png"><h3>Администрирование карточек</h3>
<style type="text/css">
    .label-important {
        background: #dd4b39;
    }
...
function ShowEdit($id) {
    return  '<button type="button" class="btn btn-default"><a href="/admin/site/CardEdit/' . $id . '">Редактировать</a></button> ';
}
function ShowName($id, $name){
    if($name == "")
        return  '<a href="/admin/site/CardEdit/' . $id . '">Редактировать</a> ';
    else
        return  '<a href="/admin/site/CardEdit/' . $id . '">'.$name.'</a> ';
}
echo CHtml::endForm();
?>
<a style="padding: 10px;background: #00c0ef;color: #f4f4f4;" href="#" target="_blank" onclick="export_send()">Экспорт в Excell</a>
...
<script type="text/javascript">
    ...
    function doStatusCard(id, st) {
        $("#curr_status").val(st);
        $("#form").attr("action", "/admin/site/CardStatus/" + id);
        $("#btn_submit").click();
    }
    ...
    function export_send() {
     ...
     function export_delete() {
        document.forms['form'].method = 'POST';
        document.forms['form'].action = "/admin/site/DeleteCard";
        document.forms['form'].submit();
    }
