...
//            $this->saveUserLang($idvac);
        ...
        $showFlash = false;
        if($flagNew){
            $message = "Ваша вакансия сохранена. Но не отображается.<br>Для того чтобы Вакансия отображалась - Вам необходимо, по надобности а также при помощи редактирования и существующих подсказок (полей для заполнения) добавить всю необходимую информацию к публикуемой вакансии.<br>После чего нажмите кнопку 'ОПУБЛИКОВАТЬ ВАКАНСИЮ'";
            $showFlash = true;
        }
        else{
            $res = Yii::app()->db->createCommand()
