...
class Articles extends Model
{
    ...
    //     $sql = "SELECT COUNT(*)
    //         FROM pages p
    //         INNER JOIN pages_content pc ON p.id = pc.page_id 
    //         WHERE p.group_id = 99"
    //     /** @var $res CDbCommand */
    ...
    public function getArticles()
    {
        ...
        $sql = "SELECT p.id, p.link
              , pc.name, pc.anons, pc.img, pc.hidden
     ...
     * Получаем новости
     */
    public function getLastArticles($inId)
    {
        $lang = Yii::app()->session['lang'];
        $sql = "SELECT p.id, p.link
...
//            $val['sname'] = Share::getShortText($val['html'], 300) ;
    ...
    public function getArticlesCount()
    {
        ...
        $sql = "SELECT COUNT(*)
            FROM pages p
            INNER JOIN pages_content pc ON p.id = pc.page_id AND lang = '{$lang}' AND pc.hidden = 0
            WHERE p.group_id =99";
        /** @var $res CDbCommand */
        $res = Yii::app()->db->createCommand($sql);
     ...
     */
    public function getArticlesSingle($inId)
    {
        $lang = Yii::app()->session['lang'];
        $sql = "SELECT pc.name, pc.html, pc.img, pc.meta_title, pc.meta_description, DATE_FORMAT(pc.crdate, '%d.%m.%Y') crdate, DATE_FORMAT(pc.pubdate, '%Y-%m-%d') pubdate, p.id
