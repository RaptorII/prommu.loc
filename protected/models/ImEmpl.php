...
//            if( Yii::app()->session['uploaduni_opts']['removeProtected'] ) foreach ($files as $key => &$val) unset($val['origProtected']);
    ...
    public function getUserMessages()
    {
        ...
        if( $idTm )
        {
            ...
            $sql = "SELECT ca.id, ca.message, ca.is_resp isresp, ca.is_read isread, ca.id_usp idusp, ca.id_use iduse, ca.files
                  , e.name nameto, e.logo phototo
                  , CONCAT(r.firstname, ' ', r.lastname) namefrom, r.photo photofrom, r.isman
    ...
    */
    ...
    {
        $arRes = $arV = $arVId = $arPid = $arVIdSelect = array();
        $this->limit = 10; // вывод 10 вакансий на странице
        // поиск вакансий

        // находим все по вакансиям
        $sql = Yii::app()->db->createCommand()
                ->select("ev.id, ev.title, ev.remdate, ed.id id_public, ct.id id_personal, vs.id_promo")
                ->from('empl_vacations ev')
                ->leftjoin('emplv_discuss ed','ed.id_vac=ev.id')
                ->leftjoin('chat_theme ct','ct.id_vac=ev.id')
                ->leftjoin(
        ...
        foreach ($sql as $v) {
            ...
            if(!empty($v['id_promo']) && !in_array($v['id_promo'], $arV[$id]['users']))
                $arV[$id]['users'][] = $v['id_promo'];
        ...
        for($i=$this->offset; $i<$nV; $i++)
            if($i<($this->offset+$this->limit))
            {
                ...
                if(count($arV[$arVId[$i]]['users']))
                    $arPid = array_merge($arPid, $arV[$arVId[$i]]['users']);

                $arRes['items'][$arVId[$i]] = $arV[$arVId[$i]];
            }
        $arPid = array_unique($arPid);
    ...
    public function getUsersData($arr)
    {
        $arRes = array();
        $sql = Yii::app()->db->createCommand()
                        ->select("id, id_user, isman, firstname, lastname, photo")
                        ->from('resume')
    ...
    public function hasAccess($chat, $id, $vacancy=0)
    {
        ...
        if(!in_array($type, [2,3]) || !intval($id))
            return false;
    ...
    public function buildMessArray($arr)
    {
        foreach ($arr as &$v)
        {
            $v['date'] = Share::getPrettyDate($v['crdate'],$v['crtime']);
            $v = $this->getUserPhoto($v);
        }
        unset($v);
    ...
    /**
     *  @param $arr [phototo,photofrom,isman,idusp]
     */
    public function getUserPhoto($arr)
    {
        $src = DS . MainConfig::$PATH_EMPL_LOGO . DS;
        if($arr['phototo'])
            $arr['phototo'] = $src . $arr['phototo'] . '100.jpg';
        else
            $arr['phototo'] = $src . MainConfig::$DEF_LOGO;

        $src = DS . MainConfig::$PATH_APPLIC_LOGO . DS;
        if($arr['photofrom'])
            $arr['photofrom'] = $src . $arr['photofrom'] . '100.jpg';
        else
            $arr['photofrom'] = $src . ($arr['isman'] ? MainConfig::$DEF_LOGO : MainConfig::$DEF_LOGO_F);

        if($arr['idusp']==$this->adminId)
            $arr['photofrom'] = '/theme/pic/prommu-adm.jpg';
