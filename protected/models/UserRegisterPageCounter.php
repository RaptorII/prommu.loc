...
class UserRegisterPageCounter extends CActiveRecord
{
   ...
   * Возвращает цели с яндексовским клиент ID по указаному периоду. Если период не указан - тащит данные за вчера
   */
  public function getYandexGoals($beginDate=false, $endDate=false)
  {
    ...
    return Yii::app()->db->createCommand()
      ->select('urpc.page, urpc.time, uc.ym_client')
      ->from('user_register_page_cnt urpc')
      ->leftJoin('user_client uc','uc.user=urpc.user')
      ->where(
        'urpc.time between :bdate and :edate AND uc.ym_client IS NOT NULL AND uc.user IS NOT NULL',
   ...
   * Возвращает цели с яндексовским клиент ID по указаному периоду. Если период не указан - тащит данные за вчера
   */
  public function getGoogleGoals($beginDate=false, $endDate=false)
  {
    ...
    return Yii::app()->db->createCommand()
      ->select('urpc.id, urpc.page, uc.client')
      ->from('user_register_page_cnt urpc')
      ->leftJoin('user_client uc','uc.user=urpc.user')
      ->where(
        'urpc.time between :bdate and :edate AND uc.user IS NOT NULL AND urpc.is_send=0',
        [':bdate'=>$bDate, ':edate'=>$eDate]
      )
      ->queryAll();
  }

  public function updateByPkArray($arUpdate,$arPk)
  {
