    ...
    */
    public function getProjectEmployer($isArcive)
    {
        ...
        foreach ($sql as $v) {
            ...
            switch ($v['status']) {
                case '0':
                    $arRes['items'][$p]['ignored']++;
                    break;
                case '1':
                    $arRes['items'][$p]['agreed']++;
                    break;
                case '-1':
                    $arRes['items'][$p]['refused']++;
                    break;
            }
        }
    ...
    */
    public function getProjectApplicant()
    {
        ...
        $sql = Yii::app()->db->createCommand()
            ...
            ->where(array('AND','pu.user = '.$idus, array('in','p.project',$arResId)))
            ->order('p.crdate desc')
            ->queryAll();
    ...
    public function exportProject($project)
    {
        ...
        $data = Yii::app()->db->createCommand()
            ...
            pc.construction, pc.building, pc.house,pc.comment, pc.post')
            ->from('project_city pc')
