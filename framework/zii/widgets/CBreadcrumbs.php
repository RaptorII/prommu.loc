...
class CBreadcrumbs extends CWidget
{
	...
	public function run()
	{
		...
		echo '<div itemscope="" itemtype="http://schema.org/BreadcrumbList" id="breadcrumbs" class="container breadcrumbs">'."\n";//CHtml::openTag($this->tagName,$this->htmlOptions)'.'\n';
		$links=array();
		...
		foreach($definedLinks as $label=>$url)
		{
			...
			if(is_string($label) || is_array($url))
				$links[]=strtr($this->activeLinkTemplate,array(
					'{url}'=>'http://' . $_SERVER['HTTP_HOST'] . CHtml::normalizeUrl($url),
		...
		echo '</div>';//CHtml::closeTag($this->tagName);
	}
}